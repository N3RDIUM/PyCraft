    ...
    def _generate_tree(self):
        ...
        * Generates the tree
        ...
        for i in range(self.height):
            self.game.add_block("OakLog", [self.position["x"], self.position["y"] + i, self.position["z"]], self.chunk)
                ...
                for j in range(-self.side, self.side+1):
                    for k in range(-self.side, self.side+1):
                        if [j,k] != [0,0] or i == self.height - 1:
                            self.game.add_block("OakLeaves", [self.position["x"]+j, self.position["y"] + i, self.position["z"]+k], self.chunk)
